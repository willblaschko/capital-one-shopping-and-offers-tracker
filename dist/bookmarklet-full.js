(()=>{var x={offers:{hostname:"capitaloneoffers",pagePath:"/c1-offers/shopping-trips",apiPattern:t=>t.includes("shopping-trips")&&t.includes("version=2")&&t.includes("_data="),apiEndpoint:null},shopping:{hostname:"capitaloneshopping",pagePath:"/account-settings/shopping-trips",apiPattern:t=>t.includes("/api/v1/trip_orders"),apiEndpoint:"/api/v1/trip_orders"}};function y(){return window.location.hostname.includes("capitaloneoffers")?"offers":window.location.hostname.includes("capitaloneshopping")?"shopping":null}function w(){let t=y();return t?window.location.pathname.startsWith(x[t].pagePath):!1}function $(t){return t?Array.isArray(t)?t:Array.isArray(t.items)?t.items:Array.isArray(t.shoppingTrips)?t.shoppingTrips:Array.isArray(t.trip_orders)?t.trip_orders:t.data&&Array.isArray(t.data)?t.data:t.data?.items&&Array.isArray(t.data.items)?t.data.items:[]:[]}function v(t){return t?Number(t)/100:null}function I(t){let r=t.orderAmount??t.order_amount??v(t.trxnTotalCents)??null,o=t.creditAmount??t.credit_amount??v(t.payoutAmountCents)??null,n=t.orderId??t.order_id??null,d=o!==null&&Number(o)>0,s=t.status??"Unknown",p=s;return d&&s.toLowerCase()==="canceled"?p="Completed":s.toLowerCase()==="pending"&&(p=d?"Pending \u2713":"Pending ?"),{id:t.id??t.tripId??null,tripId:t.tripId??t.trip_id??t.id??null,orderId:n,merchant:t.vendor??t.merchantName??t.merchant??t.merchantDisplayName??"Unknown",domain:t.domain??null,status:p,rawStatus:s,orderAmount:r!==null?Number(r):null,creditAmount:o!==null?Number(o):null,date:t.createdAt??t.created_at??t.clickDate??t.date??null,hasOrderId:n!==null,hasAmount:r!==null&&Number(r)>0,hasCreditAmount:d,raw:t}}function C(t){let o=$(t).map(I);return{trips:o,stats:{total:o.length,withOrderId:o.filter(n=>n.hasOrderId).length,withAmount:o.filter(n=>n.hasAmount).length,withCredit:o.filter(n=>n.hasCreditAmount).length,pending:o.filter(n=>n.status.toLowerCase().includes("pending")).length,created:o.filter(n=>n.status.toLowerCase()==="created").length}}}var S=`
    #c1t-fab {
...
`;function k(t){return t==null||t===0?"\u2014":"$"+Number(t).toFixed(2)}function E(t){if(!t)return"\u2014";try{return new Date(t).toLocaleDateString()}catch{return"\u2014"}}function b(t){if(t==null)return"";let r=document.createElement("div");return r.textContent=String(t),r.innerHTML}function L(t){let r=(t||"").toLowerCase();return r.includes("completed")?"completed":r==="pending \u2713"?"pending-good":r==="pending ?"||r.includes("pending")?"pending-uncertain":r.includes("created")?"created":r.includes("cancel")?"canceled":r.includes("adjust")?"adjusted":""}function A({onOpen:t,processedData:r}){let o=!1,n=r;function d(){if(o&&document.getElementById("c1t-styles"))return;let e=document.getElementById("c1t-styles");e||(e=document.createElement("style"),e.id="c1t-styles",e.textContent=S,(document.head||document.documentElement).appendChild(e)),o=!0}function s(){d();let e=document.getElementById("c1t-fab");return e||(e=document.createElement("button"),e.id="c1t-fab",e.innerHTML="\u{1F4CB}",e.title="Shopping Trips Tracker",e.addEventListener("click",async()=>{p();let i=document.getElementById("c1t-overlay");i&&i.classList.add("open"),!n&&t&&(await t(),n&&h(i,n))}),document.body.appendChild(e),n&&c(e,n),e)}function p(){d();let e=document.getElementById("c1t-overlay"),i=!1;return console.log("[C1 Tracker] ensureOverlay - existing:",!!e,"currentData:",!!n,"stats:",n?.stats),e||(i=!0,e=document.createElement("div"),e.id="c1t-overlay",e.innerHTML=`
                <div id="c1t-modal">
                    <div id="c1t-header">
            ...
            <div id="c1t-table-wrap">
                <table id="c1t-table">
                    ...
                    <tbody id="c1t-tbody">
                        ${l.map(a=>{let f=a.hasCreditAmount?"amt":a.hasOrderId?"tracked":"",u=L(a.status);return`
                                <tr class="${f}" data-filter-amount="${a.hasAmount}" data-filter-tracked="${a.hasOrderId}" data-filter-pending="${a.status.toLowerCase().includes("pending")}" data-filter-created="${a.status.toLowerCase()==="created"}">
                                    ...
                                    <td class="r ${a.hasAmount?"c1t-amount":""}">${k(a.orderAmount)}</td>
                                    <td class="r ${a.hasCreditAmount?"c1t-credit":""}">${k(a.creditAmount)}</td>
