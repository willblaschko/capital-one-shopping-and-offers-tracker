"use strict";(()=>{var I={offers:{hostname:"capitaloneoffers",pages:{trips:"/shopping-trips",browse:"/feed"},trips:{apiPattern:t=>t.includes("/xhr/shopping-trips"),apiEndpoint:"/xhr/shopping-trips?limit=100&offset=0&status[]=Activated&status[]=Adjusted&status[]=Completed&status[]=Inactive&status[]=Ineligible&status[]=Pending&status[]=Waiting"},browse:{apiPattern:t=>t.includes("/feed/")&&t.includes("viewInstanceId=")}},shopping:{hostname:"capitaloneshopping",pages:{trips:"/account-settings/shopping-trips",browse:"/"},trips:{apiPattern:t=>t.includes("/api/v1/trip_orders"),apiEndpoint:"/api/v1/trip_orders"},browse:{apiPattern:t=>t.endsWith("/api/v1/feed"),apiEndpoint:"/api/v1/feed"}}};function C(){return window.location.hostname.includes("capitaloneoffers")?"offers":window.location.hostname.includes("capitaloneshopping")?"shopping":null}function S(){let t=C();if(!t)return null;let e=window.location.pathname,r=I[t].pages;return e.startsWith(r.trips)?"trips":t==="shopping"&&(e==="/"||e==="")||t==="offers"&&e.startsWith(r.browse)?"browse":null}function J(t){if(!t)return[];if(Array.isArray(t))return t;let e=t;return Array.isArray(e.items)?e.items:Array.isArray(e.shoppingTrips)?e.shoppingTrips:Array.isArray(e.trip_orders)?e.trip_orders:e.data&&Array.isArray(e.data)?e.data:e.data&&typeof e.data=="object"&&Array.isArray(e.data.items)?e.data.items:[]}function Y(t){let e=t.orderAmount??t.order_amount??(t.trxnTotalCents!=null?t.trxnTotalCents/100:null),r=t.creditAmount??t.credit_amount??(t.payoutAmountCents!=null?t.payoutAmountCents/100:null),a=t.orderId??t.order_id??null,i=r!==null&&Number(r)>0,n=t.status??"Unknown";n==="Waiting"?n="Created":(n==="Inactive"||n==="Ineligible")&&(n="Canceled");let o=n;return i&&n.toLowerCase()==="canceled"?o="Completed":n.toLowerCase()==="pending"&&(o=i?"Pending \u2713":"Pending ?"),{id:t.id??t.tripId??t.activatedOfferId??null,tripId:t.tripId??t.trip_id??t.id??t.activatedOfferId??null,orderId:a,merchant:t.vendor??t.merchantName??t.merchantDisplayName??t.merchant??t.domain??"Unknown",domain:t.domain??null,status:o,rawStatus:n,orderAmount:e!==null?Number(e):null,creditAmount:r!==null?Number(r):null,date:t.createdAt??t.created_at??t.clickDate??t.date??null,hasOrderId:a!==null,hasAmount:e!==null&&Number(e)>0,hasCreditAmount:i,rewardDisplay:t.rewardsSummaryDisplayRate??(Array.isArray(t.rewards)?t.rewards[0]?.displayRate:void 0)??"",exclusions:t.merchantExclusions??"",raw:t}}function E(t){let r=J(t).map(Y);return{trips:r,stats:{total:r.length,withOrderId:r.filter(a=>a.hasOrderId).length,withAmount:r.filter(a=>a.hasAmount).length,withCredit:r.filter(a=>a.hasCreditAmount).length,pending:r.filter(a=>a.status.toLowerCase().includes("pending")).length,created:r.filter(a=>a.status.toLowerCase()==="created").length}}}var L=100,Q=50,Z="/xhr/shopping-trips?limit="+L+"&status[]=Activated&status[]=Adjusted&status[]=Completed&status[]=Inactive&status[]=Ineligible&status[]=Pending&status[]=Waiting";async function M(){let t=[];for(let e=0;e<Q;e++){let r=Z+"&offset="+e*L,a=await fetch(r,{method:"POST",credentials:"include"});if(!a.ok)throw new Error("shopping-trips returned "+a.status);let i=await a.json(),n=Array.isArray(i.data)?i.data:[];if(t.push(...n),i.hasMore!==!0||n.length===0)break}return{data:t}}var tt=`
    #c1t-fab {
...
`;function A(t){return t==null||t===0?"\u2014":"$"+Number(t).toFixed(2)}function et(t){if(!t)return"\u2014";try{return new Date(t).toLocaleDateString()}catch{return"\u2014"}}function d(t){if(t==null)return"";let e=document.createElement("div");return e.textContent=String(t),e.innerHTML}function nt(t){let e=(t||"").toLowerCase();return e.includes("completed")?"completed":e==="pending \u2713"?"pending-good":e==="pending ?"||e.includes("pending")?"pending-uncertain":e.includes("created")?"created":e.includes("activated")?"activated":e.includes("cancel")?"canceled":e.includes("adjust")?"adjusted":""}var B=(t,e)=>{if(console.log("[C1 Tracker] renderTripsToModal called - data:",!!e,"overlay:",!!t),!e)return;let{trips:r,stats:a}=e,i=t.querySelector("#c1t-content");console.log("[C1 Tracker] renderTripsToModal - content element:",!!i,"trips:",r?.length),i&&(i.innerHTML=`
        ...
        <div id="c1t-table-wrap">
            <table id="c1t-table">
                ...
                <tbody id="c1t-tbody">
                    ${r.map(n=>{let o=n.hasCreditAmount?"amt":n.hasOrderId?"tracked":"",s=nt(n.status),l=n.exclusions??"",u=l.length>60,g=l?u?`<div class="c1t-excl-cell" title="${d(l)}"><span class="c1t-excl-text">${d(l)}</span><button type="button" class="c1t-excl-toggle">(more)</button></div>`:`<div class="c1t-excl-cell" title="${d(l)}"><span class="c1t-excl-text">${d(l)}</span></div>`:'<span style="opacity:0.4">\u2014</span>';return`
...
... and `+(r.length-30)+" more":""}</pre>
    ...
    `,i.querySelectorAll(".c1t-filter-btn").forEach(n=>{n.addEventListener("click",function(){i.querySelectorAll(".c1t-filter-btn").forEach(s=>s.classList.remove("active")),this.classList.add("active");let o=this.dataset.filter;i.querySelectorAll("#c1t-tbody tr").forEach(s=>{if(o==="all")s.style.display="";else if(o){let l=`filter${o.charAt(0).toUpperCase()+o.slice(1)}`;s.style.display=s.dataset[l]==="true"?"":"none"}})})}),i.querySelectorAll(".c1t-excl-toggle").forEach(n=>{n.addEventListener("click",o=>{o.stopPropagation(),o.preventDefault();let s=n.closest(".c1t-excl-cell");if(!s)return;let l=s.classList.toggle("c1t-excl-expanded");n.textContent=l?"(less)":"(more)"})}))};function P(t){let{title:e,tabs:r,defaultTabId:a}=t;if(r.length===0)throw new Error("createTabbedUI: tabs must be non-empty");if(!r.find(c=>c.id===a))throw new Error(`createTabbedUI: defaultTabId "${a}" not in tabs`);let i=new Map,n=new Map,o=!1,s=a;function l(c){return r.find(p=>p.id===c)??null}function u(){if(o&&document.getElementById("c1t-styles"))return;let c=document.getElementById("c1t-styles");c||(c=document.createElement("style"),c.id="c1t-styles",c.textContent=tt,(document.head||document.documentElement).appendChild(c)),o=!0}function g(){u();let c=document.getElementById("c1t-fab");if(c)return c;let p=document.createElement("button");return p.id="c1t-fab",p.innerHTML="\u{1F4CB}",p.title=e,p.addEventListener("click",()=>{h().classList.add("open"),m(s)}),document.body.appendChild(p),k(),p}function h(){u();let c=document.getElementById("c1t-overlay");if(c)return c;c=document.createElement("div"),c.id="c1t-overlay",c.innerHTML=`
            <div id="c1t-modal">
                <div id="c1t-header">
                    <h2>\u{1F4CB} ${d(e)}</h2>
    ...
    `;let o=r.querySelector("#c1t-browse-body");o&&Rt(o),At(r),St(r)};(async function(){"use strict";let t=C();if(!t){alert("Please run this on capitaloneshopping.com or capitaloneoffers.com");return}let r=S()==="browse"?"browse":"trips";if(document.getElementById("c1t-fab")){document.getElementById("c1t-overlay")?.classList.add("open");return}console.log("[C1 Tracker Bookmarklet] Running on",t,"defaultTab=",r);async function a(){if(t==="shopping"){let s=await fetch(I.shopping.trips.apiEndpoint,{credentials:"include"});if(!s.ok)throw new Error(`API returned ${s.status}`);return E(await s.json())}return E(await M())}async function i(){let s=(h,m)=>{let b=document.querySelector("#c1t-loading");b&&(b.textContent=`Loaded ${h} pages, ${m} offers...`)};if(t==="shopping"){let h=await q(s),m=$(h.items);return m.stats.hitCap=h.hitCap,m.stats.pagesWalked=h.pagesWalked,m}let l=await V();if(!l)throw new Error("Could not capture offers feed context (userId + viewInstanceId). Open DevTools console for diagnostics. The URL should look like /feed/<userId>?viewInstanceId=<uuid>. Try clicking into the feed grid once, then re-run.");let u=await X(l,s),g=$(u.items);return g.stats.hitCap=u.hitCap,g.stats.pagesWalked=u.pagesWalked,g}let o=P({title:`${t==="offers"?"Cap One Offers":"Cap One Shopping"} Tracker`,defaultTabId:r,tabs:[{id:"trips",label:"Trips",render:B,getBadgeCount:s=>s?.stats?.withCredit??0,onActivate:a,loadingText:"Fetching shopping trips data..."},{id:"browse",label:"Browse",render:G,onActivate:i,loadingText:"Walking offers feed... (0 pages)"}]});o.ensureFab(),o.ensureOverlay(),document.getElementById("c1t-overlay")?.classList.add("open"),o.setActiveTab(r)})();})();
