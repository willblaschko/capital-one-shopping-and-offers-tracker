...
... and `+(n.length-30)+" more":""}</pre>
    ...
    </details>`}function It(t){switch(t){case"multiplier":return"Multipliers";case"percent":return"Percent";case"fixed-cash":return"Cash";case"fixed-points":return"Points"}}function Et(t){let e=[],n=new Set;for(let r of t.bucketOrder){let o=N[r];o&&(n.has(o.group)||(n.add(o.group),e.push(`<button class="c1t-jump-chip" data-jump-to="${o.id}">${d(It(o.group))}</button>`)))}return e.join("")}function Ot(t){let e=t.dataset.activationUrl;e&&window.open(e,"_blank","noopener")}async function $t(t){let e=t.dataset.activationUrl;if(!e)return;let n=t.dataset.merchant??"merchant",r=window.open("about:blank","_blank");try{let o=await fetch(e,{method:"POST",credentials:"include"});if(!o.ok)throw new Error(`Activation returned ${o.status}`);let a=await o.json(),i=a?.affiliate?.redirectUrl;if(i&&r){r.location=i;return}let s=a?.cardLinked?.cardLinkedOfferDetail;if(a?.cardLinked&&s?.isActivated){r?.close?.(),alert(`${n} card-linked offer activated. Use your card as usual \u2014 no redirect needed.`);return}if(a?.cardLinked?.cardLinkedOfferDetail?.activationLimitsReached){r?.close?.(),alert("Card-linked activation limit reached \u2014 cancel an existing activation and try again.");return}r?.close?.(),alert("Activation failed \u2014 response had no redirect and no card-linked activation.")}catch(o){r?.close?.(),alert("Activation failed: "+(o instanceof Error?o.message:String(o)))}}function Rt(t){t.addEventListener("click",e=>{let n=e.target;if(!n)return;let r=n.closest(".c1t-excl-toggle");if(r){e.stopPropagation(),e.preventDefault();let a=r.closest(".c1t-excl-cell");if(a){let i=a.classList.toggle("c1t-excl-expanded");r.textContent=i?"(less)":"(more)"}return}let o=n.closest("tr[data-method]");o&&(o.dataset.method==="href"?Ot(o):o.dataset.method==="post-offers"&&$t(o))})}function St(t){let e=t.querySelector("#c1t-browse-search input"),n=t.querySelector("#c1t-browse-search button");if(!e)return;let r=new Map;t.querySelectorAll("details[data-bucket-id]").forEach(i=>{let s=i,p=s.dataset.bucketId??"";r.set(p,s.open)});let o=null,a=i=>{let s=i.trim().toLowerCase(),p=s.length===0;t.querySelectorAll("details[data-bucket-id]").forEach(b=>{let g=b,m=g.dataset.bucketId??"",h=g.querySelectorAll("tr[data-search]"),w=0;h.forEach(k=>{let c=k.dataset.search??"",l=p||c.includes(s);k.style.display=l?"":"none",l&&w++}),w===0&&!p?g.style.display="none":(g.style.display="",p?g.open=r.get(m)??!1:g.open=!0)})};e.addEventListener("input",()=>{o&&clearTimeout(o),o=setTimeout(()=>a(e.value),100)}),n&&n.addEventListener("click",()=>{e.value="",a("")})}function At(t){let e=t.querySelector("#c1t-browse-nav");e&&e.addEventListener("click",n=>{let r=n.target;if(!r)return;let o=r.closest("[data-jump-to]");if(!o)return;let a=o.dataset.jumpTo;if(!a)return;let i=t.querySelector(`details[data-bucket-id="${a}"]`);i&&(i.open=!0,i.scrollIntoView({behavior:"smooth",block:"start"}))})}var G=(t,e)=>{let n=t.querySelector("#c1t-content");if(!n)return;let r=e.bucketOrder.map(s=>{let p=N[s];if(!p)return"";let u=e.buckets[s];return!u||!u.length?"":Ct(p,u)}).join(""),o=Et(e),a=e.stats.hitCap?`Stopped at ${e.stats.total} items (max pages reached)`:`${e.stats.total} offers across ${e.bucketOrder.length} buckets`;n.innerHTML=`
