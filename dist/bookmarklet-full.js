...
`;function R(t){return t==null||t===0?"\u2014":"$"+Number(t).toFixed(2)}function Y(t){if(!t)return"\u2014";try{return new Date(t).toLocaleDateString()}catch{return"\u2014"}}function d(t){if(t==null)return"";let e=document.createElement("div");return e.textContent=String(t),e.innerHTML}function Z(t){let e=(t||"").toLowerCase();return e.includes("completed")?"completed":e==="pending \u2713"?"pending-good":e==="pending ?"||e.includes("pending")?"pending-uncertain":e.includes("created")?"created":e.includes("cancel")?"canceled":e.includes("adjust")?"adjusted":""}var M=(t,e)=>{if(console.log("[C1 Tracker] renderTripsToModal called - data:",!!e,"overlay:",!!t),!e)return;let{trips:n,stats:a}=e,o=t.querySelector("#c1t-content");console.log("[C1 Tracker] renderTripsToModal - content element:",!!o,"trips:",n?.length),o&&(o.innerHTML=`
        ...
        <div id="c1t-table-wrap">
            <table id="c1t-table">
                ...
                <tbody id="c1t-tbody">
                    ${n.map(r=>{let i=r.hasCreditAmount?"amt":r.hasOrderId?"tracked":"",s=Z(r.status);return`
...
... and `+(n.length-30)+" more":""}</pre>
    ...
    `,o.querySelectorAll(".c1t-filter-btn").forEach(r=>{r.addEventListener("click",function(){o.querySelectorAll(".c1t-filter-btn").forEach(s=>s.classList.remove("active")),this.classList.add("active");let i=this.dataset.filter;o.querySelectorAll("#c1t-tbody tr").forEach(s=>{if(i==="all")s.style.display="";else if(i){let p=`filter${i.charAt(0).toUpperCase()+i.slice(1)}`;s.style.display=s.dataset[p]==="true"?"":"none"}})})}))};function P(t){let{title:e,tabs:n,defaultTabId:a}=t;if(n.length===0)throw new Error("createTabbedUI: tabs must be non-empty");if(!n.find(c=>c.id===a))throw new Error(`createTabbedUI: defaultTabId "${a}" not in tabs`);let o=new Map,r=new Map,i=!1,s=a;function p(c){return n.find(l=>l.id===c)??null}function u(){if(i&&document.getElementById("c1t-styles"))return;let c=document.getElementById("c1t-styles");c||(c=document.createElement("style"),c.id="c1t-styles",c.textContent=Q,(document.head||document.documentElement).appendChild(c)),i=!0}function b(){u();let c=document.getElementById("c1t-fab");if(c)return c;let l=document.createElement("button");return l.id="c1t-fab",l.innerHTML="\u{1F4CB}",l.title=e,l.addEventListener("click",()=>{g().classList.add("open"),m(s)}),document.body.appendChild(l),k(),l}function g(){u();let c=document.getElementById("c1t-overlay");if(c)return c;c=document.createElement("div"),c.id="c1t-overlay",c.innerHTML=`
        ...
        `,document.body.appendChild(c);let l=c;return l.querySelector("#c1t-close")?.addEventListener("click",()=>{l.classList.remove("open")}),l.addEventListener("click",f=>{f.target===l&&l.classList.remove("open")}),l.querySelectorAll(".c1t-tab").forEach(f=>{f.addEventListener("click",()=>{let x=f.dataset.tabId;x&&m(x)})}),c}async function m(c){let l=p(c);if(!l)return;s=c;let f=document.getElementById("c1t-overlay");f&&f.querySelectorAll(".c1t-tab").forEach(y=>{y.classList.toggle("active",y.dataset.tabId===c)});let x=f?.querySelector("#c1t-content");if(o.has(c)){x&&l.render(f,o.get(c));return}if(!l.onActivate){x&&(x.innerHTML=`<div id="c1t-loading">${d(l.loadingText??"No data.")}</div>`);return}if(r.has(c)){await r.get(c);return}x&&(x.innerHTML=`<div id="c1t-loading">${d(l.loadingText??"Loading\u2026")}</div>`);let v=(async()=>{try{let y=await l.onActivate();y!=null&&w(c,y)}catch(y){console.error("[C1 Tracker] tab loader threw:",y);let V=y instanceof Error?y.message:String(y),S=document.getElementById("c1t-content");S&&s===c&&(S.innerHTML=`<div id="c1t-loading">Error loading data: ${d(V)}</div>`)}finally{r.delete(c)}})();r.set(c,v),await v}function h(c){m(c)}function w(c,l){let f=p(c);if(!f)return;o.set(c,l),k();let x=document.getElementById("c1t-overlay");x&&s===c&&f.render(x,l)}function k(){let c=document.getElementById("c1t-fab");if(!c)return;let l=0,f=!1;for(let x of n){if(!o.has(x.id)||(f=!0,!x.getBadgeCount))continue;let v=x.getBadgeCount(o.get(x.id));v>l&&(l=v)}f?c.classList.add("has-data"):c.classList.remove("has-data"),c.innerHTML=l>0?`\u{1F4CB}<span class="badge">${l}</span>`:"\u{1F4CB}"}return document.addEventListener("keydown",c=>{if(c.key==="Escape"){let l=document.getElementById("c1t-overlay");l&&l.classList.remove("open")}}),{ensureStyles:u,ensureFab:b,ensureOverlay:g,setActiveTab:h,setTabData:w,getActiveTabId:()=>s}}var tt=/(\d+(?:\.\d+)?)X/i,et=/(\d+(?:\.\d+)?)%/,nt=/\$([\d,]+(?:\.\d+)?)/,rt=/([\d,]+)\s*(miles|points)/i;function T(t){let e=String(t??""),n=e.trim();if(!n)return{type:"unknown",value:0,display:e};let a=n.match(tt);if(a&&a[1]!==void 0)return{type:"multiplier",value:parseFloat(a[1]),display:e};let o=n.match(nt);if(o&&o[1]!==void 0)return{type:"fixed-cash",value:parseFloat(o[1].replace(/,/g,"")),display:e};let r=n.match(rt);if(r&&r[1]!==void 0)return{type:"fixed-points",value:parseFloat(r[1].replace(/,/g,"")),display:e};let i=n.match(et);return i&&i[1]!==void 0?{type:"percent",value:parseFloat(i[1]),display:e}:{type:"unknown",value:0,display:e}}function F(t){let e=t.stats??{};return e.cashbackV2??e.cashback??e.cashbackAmount??""}function at(t){if(!t||!t.length)return null;let e=null;for(let n of t){let a=T(n.cashback);a.value>0&&(!e||a.value>e.value)&&(e={value:a.value,display:n.cashback})}return e}function ot(t){switch(t){case"great_deal":return"price-drops";case"event_placement":return"events";case"nca_deal":return"new-customer";case"retarget":case"retarget_non_product":return"recently-viewed";default:return"value"}}function it(t){if(!t.href)return null;let e=t.merchantName??"",n=t.domain??"";if(!e&&!n)return null;let a=t.stats??{},o=a.isCutType===!0||a.rewardType==="cut",r,i,s;if(o){let m=at(a.cashbackCategories);if(m){r="percent",i=m.value;let h=m.display.trim();s=h.toLowerCase().startsWith("up to")?h:"Up to "+h}else{let h=T(F(t));r=h.type,i=h.value,s=h.display.toLowerCase().startsWith("up to")?h.display:h.value?"Up to "+h.display:h.display}}else{let m=T(F(t));r=m.type,i=m.value,s=m.display}let p={method:"href",url:t.href},u=ot(t.type),b=t.id??null;return{id:b!==null?String(b):`shopping|${e||n}|${s}|${t.type}`,source:"shopping",itemType:t.type,merchant:e||n,domain:n||e,rewardType:r,rewardValue:i,rewardDisplay:s,activation:p,bucketCategory:u,pill:t.pill?.text??null,exclusions:a.exclusionsText??"",eventEnd:t.end??null,priceHistory:a.priceHistory??null,raw:t}}function st(t,e){return`https://capitaloneoffers.com/feed/${encodeURIComponent(t.userId)}/offers/${e}?_data`}function H(t,e){if(t.type==="Carousel"){let s=t.tiles??[],p=[];for(let u of s)for(let b of H(u,e))p.push(b);return p}let n=t.id,a=t.merchantTLD;if(!n||!a)return[];let o=t.buttonText??"",r=T(o),i=t.subText&&t.headingText?`${t.headingText} \u2014 ${t.subText}`:t.subText??t.headingText??t.text??"";return[{id:n,source:"offers",itemType:t.type,merchant:a,domain:a,rewardType:r.type,rewardValue:r.value,rewardDisplay:r.display,activation:{method:"post-offers",url:st(e,n)},bucketCategory:"value",pill:t.badge?.text??null,exclusions:i,eventEnd:null,priceHistory:null,raw:t}]}function ct(t){let e=t.rewardValue;switch(t.rewardType){case"multiplier":return e>=30?"mult-30":e>=20?"mult-20":e>=10?"mult-10":"mult-1";case"percent":case"cut":return e>=40?"pct-40":e>=20?"pct-20":e>=10?"pct-10":"pct-1";case"fixed-cash":return e>=50?"cash-50":e>=25?"cash-25":"cash-0";case"fixed-points":return e>=1e4?"pts-10k":e>=5e3?"pts-5k":e>=1e3?"pts-1k":"pts-lt-1k";case"unknown":default:return"pct-1"}}var _=[{id:"mult-30",label:"Multipliers \xB7 30X+",group:"multiplier",initiallyOpen:!0},{id:"mult-20",label:"Multipliers \xB7 20\u201329X",group:"multiplier",initiallyOpen:!0},{id:"mult-10",label:"Multipliers \xB7 10\u201319X",group:"multiplier",initiallyOpen:!1},{id:"mult-1",label:"Multipliers \xB7 1\u20139X",group:"multiplier",initiallyOpen:!1},{id:"pct-40",label:"Percent \xB7 40%+",group:"percent",initiallyOpen:!0},{id:"pct-20",label:"Percent \xB7 20\u201339%",group:"percent",initiallyOpen:!0},{id:"pct-10",label:"Percent \xB7 10\u201319%",group:"percent",initiallyOpen:!1},{id:"pct-1",label:"Percent \xB7 1\u20139%",group:"percent",initiallyOpen:!1},{id:"cash-50",label:"Fixed Cash \xB7 $50+",group:"fixed-cash",initiallyOpen:!0},{id:"cash-25",label:"Fixed Cash \xB7 $25\u201349",group:"fixed-cash",initiallyOpen:!0},{id:"cash-0",label:"Fixed Cash \xB7 under $25",group:"fixed-cash",initiallyOpen:!1},{id:"pts-10k",label:"Fixed Points \xB7 10,000+",group:"fixed-points",initiallyOpen:!0},{id:"pts-5k",label:"Fixed Points \xB7 5,000\u20139,999",group:"fixed-points",initiallyOpen:!0},{id:"pts-1k",label:"Fixed Points \xB7 1,000\u20134,999",group:"fixed-points",initiallyOpen:!1},{id:"pts-lt-1k",label:"Fixed Points \xB7 under 1,000",group:"fixed-points",initiallyOpen:!1}],U=(()=>{let t={};for(let e of _)t[e.id]=e;return t})();function $(t){let e={};for(let r of t){let i=ct(r);(e[i]??(e[i]=[])).push(r)}for(let r of Object.keys(e))e[r].sort((i,s)=>s.rewardValue-i.rewardValue);let n=[],a={};for(let r of _){let i=e[r.id];i&&i.length&&(n.push(r.id),a[r.id]=i.length)}let o={total:t.length,byBucket:a};return{offers:t,buckets:e,bucketOrder:n,stats:o}}async function j(t){let e=t.maxPages??40,n=new Set,a=[],o=null,r=0;for(;r<e;){let i=await t.fetchPage(o);if(!i)break;for(let p of t.getItems(i)){let u=t.dedupeKey(p);u&&n.has(u)||(u&&n.add(u),a.push(p))}r++,t.onPage?.(r,a.length);let s=t.getNextCursor(i);if(!s)break;o=s}return{items:a,hitCap:r>=e,pagesWalked:r}}function lt(t){let e={limit:25};return t&&(e.nextPageToken=t),JSON.stringify({contentProps:{pagination:e},context:{device:{model:typeof navigator<"u"&&/Mac/.test(navigator.platform)?"Macintosh":"Unknown",manufacturer:"Unknown",memory:"8",concurrency:String(typeof navigator<"u"&&navigator.hardwareConcurrency||4)},browser:{name:"Chrome",version:"0",major:"0"},os:{name:"unknown",version:"0"},screen:{width:1920,height:1080,density:2},locale:typeof navigator<"u"&&navigator.language?navigator.language:"en-US",country:"US",location:{state:"",zipcode:"",latitude:null,longitude:null,isInCensusData:!1},page:{path:typeof window<"u"?window.location.pathname:"/",url:typeof window<"u"?window.location.href:"",referrer:typeof document<"u"?document.referrer:"",search:typeof window<"u"?window.location.search:"",title:typeof document<"u"?document.title:""},userAgent:typeof navigator<"u"?navigator.userAgent:""}})}function pt(t){let e=t;if(e.id!==void 0&&e.id!==null&&e.id!=="")return String(e.id);let n=t.merchantName??"",a=t.stats?.cashbackV2??t.stats?.cashback??"";return!n&&!a?null:`${n}|${a}|${t.type}`}async function z(t){let e={fetchPage:async r=>{let i=await fetch("/api/v1/feed",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:lt(r)});if(!i.ok)return console.warn("[C1 Tracker] shopping feed POST failed",{status:i.status,statusText:i.statusText,cursor:r}),null;let s=await i.json();return r||console.log("[C1 Tracker] shopping feed first page",{count:s.count,itemCount:s.items?.length??0,nextPageToken:s.pagination?.nextPageToken}),s},getNextCursor:r=>r.pagination?.nextPageToken??null,getItems:r=>r.items??[],dedupeKey:pt,...t?{onPage:t}:{},maxPages:40},n=await j(e),a=[],o=0;for(let r of n.items){let i=it(r);i?a.push(i):o++}return console.log("[C1 Tracker] shopping walk done",{rawItems:n.items.length,normalized:a.length,droppedDuringNormalize:o,pagesWalked:n.pagesWalked,hitCap:n.hitCap}),{items:a,hitCap:n.hitCap,pagesWalked:n.pagesWalked}}function dt(t,e){let n=`https://capitaloneoffers.com/feed/${encodeURIComponent(t.userId)}`,a=`?numberOfColumnsInGrid=5&viewInstanceId=${t.viewInstanceId}&contentSlug=ease-web-l1`;return e?`${n}${a}&cursor=${e}`:`${n}${a}`}function ut(t){let e=t.merchantTLD??"",n=t.buttonText??"";return e&&n?`${e}|${n}`:t.id??null}function mt(t){let e=[];for(let n of t)if(n.type==="Carousel")for(let a of n.tiles??[])e.push(a);else e.push(n);return e}async function W(t,e){let n={fetchPage:async r=>{let i=await fetch(dt(t,r),{method:"GET",credentials:"include",headers:{Accept:"application/json"}});return i.ok?await i.json():null},getNextCursor:r=>r.cursor??null,getItems:r=>mt(r.data??[]),dedupeKey:ut,...e?{onPage:e}:{},maxPages:40},a=await j(n),o=[];for(let r of a.items)for(let i of H(r,t))o.push(i);return{items:o,hitCap:a.hitCap,pagesWalked:a.pagesWalked}}function O(t,e,n=0){if(n>6||t===null||typeof t!="object")return null;let a=t;for(let o of e){let r=a[o];if(typeof r=="string"&&r.length>0)return r}for(let o of Object.keys(a)){let r=a[o];if(r&&typeof r=="object"){let i=O(r,e,n+1);if(i)return i}}return null}function D(t){let e=new RegExp(`\\\\?"${t}\\\\?"\\s*,\\s*\\\\?"([^"\\\\]+)\\\\?"`),n=document.getElementsByTagName("script");for(let a=0;a<n.length;a++){let o=n[a].textContent;if(!o||o.indexOf(t)<0)continue;let r=o.match(e);if(r&&r[1])return r[1]}return null}function ft(){let t=null,e=null;try{e=new URLSearchParams(window.location.search).get("viewInstanceId")}catch{}let n=window.location.pathname.match(/^\/feed\/([^/?#]+)/);if(n&&n[1]&&(t=decodeURIComponent(n[1])),t||(t=D("maybeSelectedArid")),e||(e=D("viewInstanceId")),!t||!e)try{let a=document.getElementById("__NEXT_DATA__");if(a?.textContent){let o=JSON.parse(a.textContent);t||(t=O(o,["userId","accountReferenceId"])),e||(e=O(o,["viewInstanceId"]))}}catch{}if(!e&&t)try{typeof crypto<"u"&&typeof crypto.randomUUID=="function"&&(e=crypto.randomUUID())}catch{}return t&&e?{userId:t,viewInstanceId:e}:(console.warn("[C1 Tracker] getOffersBrowseContext (sync) failed",{pathname:window.location.pathname,search:window.location.search,userId:t,viewInstanceId:e,hasNextData:!!document.getElementById("__NEXT_DATA__")}),null)}async function N(){let t=ft();if(t)return t;let e=null,n=null;try{n=new URLSearchParams(window.location.search).get("viewInstanceId")}catch{}try{let a=await fetch("/xhr/shopping-trips?limit=1&offset=0&status[]=Adjusted&status[]=Completed&status[]=Ineligible&status[]=Pending",{method:"POST",credentials:"include"});if(a.ok){let r=(await a.json())?.data?.[0];r&&typeof r.accountReferenceId=="string"&&(e=r.accountReferenceId)}}catch(a){console.warn("[C1 Tracker] trips-API fallback for userId failed:",a)}if(!n&&e)try{typeof crypto<"u"&&typeof crypto.randomUUID=="function"&&(n=crypto.randomUUID())}catch{}return e&&n?{userId:e,viewInstanceId:n}:(console.warn("[C1 Tracker] fetchOffersBrowseContext failed",{userId:e,viewInstanceId:n}),null)}function gt(t,e){return e==="events"?"event":e==="price-drops"?"deal":e==="new-customer"?"new":e==="recently-viewed"?"retarget":t==="great_deal"?"deal":""}function ht(t){return`${t.merchant} ${t.domain} ${t.rewardDisplay} ${t.itemType} ${t.exclusions}`.toLowerCase()}function bt(t){if(!t)return"";try{return new Date(t).toLocaleDateString(void 0,{month:"short",day:"numeric"})}catch{return""}}function xt(t,e){let n=e.map(o=>{let r=d(ht(o)),i=o.pill?`<span class="c1t-pill ${gt(o.itemType,o.bucketCategory)}">${d(o.pill)}</span>`:"",s=o.eventEnd?`<span class="c1t-event-end">ends ${d(bt(o.eventEnd))}</span>`:"",p=o.exclusions??"",u=p?` title="${d(p)}"`:"",b=p?d(p):"",g=p.length>60,m=b?g?`<div class="c1t-excl-cell"${u}>
    ...
    </details>`}function yt(t){switch(t){case"multiplier":return"Multipliers";case"percent":return"Percent";case"fixed-cash":return"Cash";case"fixed-points":return"Points"}}function wt(t){let e=[],n=new Set;for(let a of t.bucketOrder){let o=U[a];o&&(n.has(o.group)||(n.add(o.group),e.push(`<button class="c1t-jump-chip" data-jump-to="${o.id}">${d(yt(o.group))}</button>`)))}return e.join("")}function kt(t){let e=t.dataset.activationUrl;e&&window.open(e,"_blank","noopener")}async function vt(t){let e=t.dataset.activationUrl;if(!e)return;let n=window.open("about:blank","_blank");try{let r=(await(await fetch(e,{method:"POST",credentials:"include"})).json())?.affiliate?.redirectUrl;r&&n?n.location=r:n&&(n.close?.(),alert("Activation failed \u2014 try clicking the tile on Cap One directly."))}catch(a){n?.close?.(),alert("Activation failed: "+(a instanceof Error?a.message:String(a)))}}function Tt(t){t.addEventListener("click",e=>{let n=e.target;if(!n)return;let a=n.closest(".c1t-excl-toggle");if(a){e.stopPropagation(),e.preventDefault();let r=a.closest(".c1t-excl-cell");if(r){let i=r.classList.toggle("c1t-excl-expanded");a.textContent=i?"(less)":"(more)"}return}let o=n.closest("tr[data-method]");o&&(o.dataset.method==="href"?kt(o):o.dataset.method==="post-offers"&&vt(o))})}function Ct(t){let e=t.querySelector("#c1t-browse-search input"),n=t.querySelector("#c1t-browse-search button");if(!e)return;let a=new Map;t.querySelectorAll("details[data-bucket-id]").forEach(i=>{let s=i,p=s.dataset.bucketId??"";a.set(p,s.open)});let o=null,r=i=>{let s=i.trim().toLowerCase(),p=s.length===0;t.querySelectorAll("details[data-bucket-id]").forEach(b=>{let g=b,m=g.dataset.bucketId??"",h=g.querySelectorAll("tr[data-search]"),w=0;h.forEach(k=>{let c=k.dataset.search??"",l=p||c.includes(s);k.style.display=l?"":"none",l&&w++}),w===0&&!p?g.style.display="none":(g.style.display="",p?g.open=a.get(m)??!1:g.open=!0)})};e.addEventListener("input",()=>{o&&clearTimeout(o),o=setTimeout(()=>r(e.value),100)}),n&&n.addEventListener("click",()=>{e.value="",r("")})}function It(t){let e=t.querySelector("#c1t-browse-nav");e&&e.addEventListener("click",n=>{let a=n.target;if(!a)return;let o=a.closest("[data-jump-to]");if(!o)return;let r=o.dataset.jumpTo;if(!r)return;let i=t.querySelector(`details[data-bucket-id="${r}"]`);i&&(i.open=!0,i.scrollIntoView({behavior:"smooth",block:"start"}))})}var q=(t,e)=>{let n=t.querySelector("#c1t-content");if(!n)return;let a=e.bucketOrder.map(s=>{let p=U[s];if(!p)return"";let u=e.buckets[s];return!u||!u.length?"":xt(p,u)}).join(""),o=wt(e),r=e.stats.hitCap?`Stopped at ${e.stats.total} items (max pages reached)`:`${e.stats.total} offers across ${e.bucketOrder.length} buckets`;n.innerHTML=`
    ...
    `;let i=n.querySelector("#c1t-browse-body");i&&Tt(i),Ct(n),It(n)};(async function(){"use strict";let t=I();if(!t){alert("Please run this on capitaloneshopping.com or capitaloneoffers.com");return}let n=L()==="browse"?"browse":"trips";if(document.getElementById("c1t-fab")){document.getElementById("c1t-overlay")?.classList.add("open");return}console.log("[C1 Tracker Bookmarklet] Running on",t,"defaultTab=",n);async function a(){if(t==="shopping"){let s=await fetch(C.shopping.trips.apiEndpoint,{credentials:"include"});if(!s.ok)throw new Error(`API returned ${s.status}`);return E(await s.json())}return E(await B())}async function o(){let s=(g,m)=>{let h=document.querySelector("#c1t-loading");h&&(h.textContent=`Loaded ${g} pages, ${m} offers...`)};if(t==="shopping"){let g=await z(s),m=$(g.items);return m.stats.hitCap=g.hitCap,m.stats.pagesWalked=g.pagesWalked,m}let p=await N();if(!p)throw new Error("Could not capture offers feed context (userId + viewInstanceId). Open DevTools console for diagnostics. The URL should look like /feed/<userId>?viewInstanceId=<uuid>. Try clicking into the feed grid once, then re-run.");let u=await W(p,s),b=$(u.items);return b.stats.hitCap=u.hitCap,b.stats.pagesWalked=u.pagesWalked,b}let i=P({title:`${t==="offers"?"Cap One Offers":"Cap One Shopping"} Tracker`,defaultTabId:n,tabs:[{id:"trips",label:"Trips",render:M,getBadgeCount:s=>s?.stats?.withCredit??0,onActivate:a,loadingText:"Fetching shopping trips data..."},{id:"browse",label:"Browse",render:q,onActivate:o,loadingText:"Walking offers feed... (0 pages)"}]});i.ensureFab(),i.ensureOverlay(),document.getElementById("c1t-overlay")?.classList.add("open"),i.setActiveTab(n)})();})();
