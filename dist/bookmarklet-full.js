...
`;function v(t){return t==null||t===0?"\u2014":"$"+Number(t).toFixed(2)}function E(t){if(!t)return"\u2014";try{return new Date(t).toLocaleDateString()}catch{return"\u2014"}}function b(t){if(t==null)return"";let r=document.createElement("div");return r.textContent=String(t),r.innerHTML}function I(t){let r=(t||"").toLowerCase();return r.includes("completed")?"completed":r==="pending \u2713"?"pending-good":r==="pending ?"||r.includes("pending")?"pending-uncertain":r.includes("created")?"created":r.includes("cancel")?"canceled":r.includes("adjust")?"adjusted":""}function C({onOpen:t,processedData:r}){let o=!1,n=r;function c(){if(o&&document.getElementById("c1t-styles"))return;let e=document.getElementById("c1t-styles");e||(e=document.createElement("style"),e.id="c1t-styles",e.textContent=$,(document.head||document.documentElement).appendChild(e)),o=!0}function s(){c();let e=document.getElementById("c1t-fab");return e||(e=document.createElement("button"),e.id="c1t-fab",e.innerHTML="\u{1F4CB}",e.title="Shopping Trips Tracker",e.addEventListener("click",async()=>{p();let i=document.getElementById("c1t-overlay");i&&i.classList.add("open"),!n&&t&&(await t(),n&&u(i,n))}),document.body.appendChild(e),n&&d(e,n),e)}function p(){c();let e=document.getElementById("c1t-overlay"),i=!1;return console.log("[C1 Tracker] ensureOverlay - existing:",!!e,"currentData:",!!n,"stats:",n?.stats),e||(i=!0,e=document.createElement("div"),e.id="c1t-overlay",e.innerHTML=`
                <div id="c1t-modal">
                    <div id="c1t-header">
            ...
            `,document.body.appendChild(e),e.querySelector("#c1t-close").addEventListener("click",()=>e.classList.remove("open")),e.addEventListener("click",l=>{l.target===e&&e.classList.remove("open")})),console.log("[C1 Tracker] ensureOverlay - isNew:",i,"currentData:",!!n,"stats:",n?.stats),n&&u(e,n),e}function d(e,i){i&&(e.classList.add("has-data"),i.stats.withCredit>0?e.innerHTML=`\u{1F4CB}<span class="badge">${i.stats.withCredit}</span>`:e.innerHTML="\u{1F4CB}")}function u(e,i){if(console.log("[C1 Tracker] renderDataToModal called - data:",!!i,"overlay:",!!e),!i)return;let{trips:l,stats:m}=i,f=e.querySelector("#c1t-content");console.log("[C1 Tracker] renderDataToModal - content element:",!!f,"trips:",l?.length),f&&(f.innerHTML=`
            <div id="c1t-stats">
            ...
            <div id="c1t-table-wrap">
                <table id="c1t-table">
                    ...
                    <tbody id="c1t-tbody">
                        ${l.map(a=>{let h=a.hasCreditAmount?"amt":a.hasOrderId?"tracked":"",g=I(a.status);return`
                                <tr class="${h}" data-filter-amount="${a.hasAmount}" data-filter-tracked="${a.hasOrderId}" data-filter-pending="${a.status.toLowerCase().includes("pending")}" data-filter-created="${a.status.toLowerCase()==="created"}">
                                    <td title="${b(a.domain)}">${b(a.merchant)}</td>
                                    <td class="c">${E(a.date)}</td>
                                    <td class="r ${a.hasAmount?"c1t-amount":""}">${v(a.orderAmount)}</td>
                                    <td class="r ${a.hasCreditAmount?"c1t-credit":""}">${v(a.creditAmount)}</td>
                                    <td class="c"><span class="c1t-status ${g}">${b(a.status)}</span></td>
...
... and `+(l.length-30)+" more":""}</pre>
        ...
        `,f.querySelectorAll(".c1t-filter-btn").forEach(a=>{a.addEventListener("click",function(){f.querySelectorAll(".c1t-filter-btn").forEach(g=>g.classList.remove("active")),this.classList.add("active");let h=this.dataset.filter;f.querySelectorAll("#c1t-tbody tr").forEach(g=>{if(h==="all")g.style.display="";else{let A=`filter${h.charAt(0).toUpperCase()+h.slice(1)}`;g.style.display=g.dataset[A]==="true"?"":"none"}})})}))}return document.addEventListener("keydown",e=>{if(e.key==="Escape"){let i=document.getElementById("c1t-overlay");i&&i.classList.remove("open")}}),{ensureStyles:c,ensureFab:s,ensureOverlay:p,updateFabState:d,renderDataToModal:u,updateData(e){console.log("[C1 Tracker] updateData called with stats:",e?.stats),n=e;let i=document.getElementById("c1t-fab");i&&d(i,e);let l=document.getElementById("c1t-overlay");l&&u(l,e)}}}(async function(){"use strict";let t=y();if(!t){alert("Please run this on capitaloneshopping.com or capitaloneoffers.com");return}if(!w()){let s=x[t].pagePath;alert(`Please navigate to the Shopping Trips page first:
${window.location.origin}${s}`);return}if(document.getElementById("c1t-fab")){let s=document.getElementById("c1t-overlay");s&&s.classList.add("open");return}console.log("[C1 Tracker Bookmarklet] Running on",t);let r=null,o=C({processedData:null,onOpen:()=>{r||c()}});o.ensureFab(),o.ensureOverlay();let n=document.getElementById("c1t-overlay");n&&n.classList.add("open");async function c(){let s=document.querySelector("#c1t-content");s&&(s.innerHTML='<div id="c1t-loading">Fetching shopping trips data...</div>');try{let p;if(t==="shopping"){let d=await fetch("/api/v1/trip_orders",{credentials:"include"});if(!d.ok)throw new Error(`API returned ${d.status}`);p=await d.json()}else{let d=await fetch("/api/shopping-trips?version=2",{credentials:"include"});if(d.ok)p=await d.json();else{let u=await fetch("/shopping-trips?_data=routes%2Fshopping-trips&version=2",{credentials:"include"});if(!u.ok)throw new Error("Could not fetch data. Navigate to Shopping Trips page and try again.");p=await u.json()}}console.log("[C1 Tracker Bookmarklet] Fetched data:",p),r=k(p),console.log("[C1 Tracker Bookmarklet] Processed:",r.stats),o.updateData(r)}catch(p){console.error("[C1 Tracker Bookmarklet] Error:",p),s&&(s.innerHTML=`
                    <div id="c1t-loading">
